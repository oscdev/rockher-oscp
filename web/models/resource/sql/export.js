...
export const bdModelExport = {
    ...
    getCollectionData: async function (collection) {
        try {
            console.log("collection_id", collection.collection_id)
            // const session = await Shopify.Utils.loadCurrentSession(req,	res, false);
            return new Promise((resolve, reject) => {
                connection.query(`SELECT p.prod_id, p.collection_id,  SUBSTRING_INDEX(m.sku, '-', 1) sku FROM productss AS p INNER JOIN metafieldss AS m ON p.prod_id = m.product_id AND p.collection_id = m.collection_id AND p.timestamp = m.timestamp  WHERE p.collection_id = '${collection.collection_id}' AND p.shop = '${collection.shop}';`, function (error, results, fields) {
                    if (error) {
                        console.warn(error);
                        resolve();
                    }
    ...
    getCollectionDataBackup: async function (collection) {
        try {
            console.log("collection_id", collection.collection_id)
            // const session = await Shopify.Utils.loadCurrentSession(req,	res, false);
            return new Promise((resolve, reject) => {
                connection.query(`SELECT prod_id, collection_id FROM productsbackup ;`, function (error, results, fields) {
                    if (error) {
                        console.warn(error);
                        resolve();
                    }
    ...
    getCollectionID: async function (req, res) {
        try {
            const session = await Shopify.Utils.loadCurrentSession(req, res, false);
            return new Promise((resolve, reject) => {
                connection.query(`SELECT * FROM collections WHERE status = 'PENDING' AND shop = '${session.shop}' LIMIT 1;`, function (error, results, fields) {
                    if (error) {
                        console.warn(error);
                        resolve();
                    }
    ...
    collectionStatusUpdate: async function (collectionID, status) {
        try {
            ...
            return new Promise((resolve, reject) => {
                connection.query(`UPDATE collections SET status = '${status}' WHERE collection_id = '${collectionID}'`, function (error, results, fields) {
                    if (error) {
                        console.warn(error);
                        resolve();
                    }
