...
export default function applyAuthMiddleware(
  ...
  app.get("/api/auth/callback", async (req, res) => {
    ...
    try {
      ...
      //Deprecated
      //setScriptTag(session.shop, session.accessToken);

      /*const client = new Shopify.Clients.Rest(session.shop, session.accessToken);
      const scriptData = await client.post({
        ...
        data : {
          script_tag : {
            ...
            src : (isProd) ? `https://${Shopify.Context.HOST_NAME}/appfront/appjs` : 'https://'+session.shop+'/apps/tier/assets/appjs.js'
          }
        }
      });*/
      ...
      /*var selectQueryAll = "SELECT * FROM SHOPIFY_SESSIONS ;";

      db.all(selectQueryAll , async (err , data) => {
        if(err){
          console.log('db error ===', err);
        } else {
          console.log('all session sql  ===', data);
        }
      });*/
